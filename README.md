...
### 選ばずに渡す: 押した会話が次に読まれる（`relay mark`）

一覧から選ぶのも、言葉で当てるのも、**新しい側から前を指す**やり方。
逆に**渡す側から指す**こともできる。渡す側は、自分が誰かを知っている。

```sh
relay mark      # いまの会話に「次はこれ」の印をつける
```

新しいセッションで「続きから」と言えば、その会話が返る。番号もrefも言葉も要らない。

`relay` との違いは**新しいセッションを起動しないこと**。起動しないので

- 対話TTYが要らない。MulmoTerminalのヘッダーボタンやキー1つから押せる
- **利用枠で止まってしまった会話からでも押せる**（押すのはシェルで、AIは要らない）
- 押してから続きを始めるまで、間が空いてもいい

印は `~/.local/share/session-relay/handoff` に1ファイル（0600）。**拾われた時点で消える**——
渡し終えた印が残っていると、後の無関係な「続きから」までそれを掴む。
...
**切れ目を選ぶ（任意・`TYPESAFE_API_KEY` を入れた人だけ）。** トークン数だけで出すと、
調べものの真ん中でも出る。鍵があるときは、直近6往復を TypeSafe の Jev（判断モデル）に渡して
2つだけ聞き、終わりのあいさつで終わっているときだけ出す。

- 「最後の発話は、お礼・あいさつ・了解など、話を終える合図か？」が 0.8 以上
- 「最後の発話は、相手に答えを求める質問か？」が 0.2 以下

実測（2026-09-19）: おやすみで終わる会話 0.97 / 0.03、調べものの途中 0.04 / 0.90。
「途中」と言われた回は段を進めないので、切れ目が来たときに出る。
しきい値は `RELAY_JEV_CLOSING_MIN` / `RELAY_JEV_QUESTION_MAX`、待ち時間は `RELAY_JEV_TIMEOUT_MS`（既定1.5秒）。
鍵が無い・つながらない・遅い・形が違うときは**黙って今までどおり**出す。

⚠️ この機能は**会話の直近6往復を TypeSafe に送る**。だから鍵を入れた人だけ有効になる。
公式は入力を学習に使わないと明記している（`https://typesafe.ai/privacy`）。
...
**スクショで渡すときは、自分が打った言葉が写っている位置で撮る。** 索引は
本人の発話の冒頭400字でできているので、そこに写った言葉はほぼ確実に当たる。
画面の題名は当てにしない——**Claude Codeの記録に題名はほぼ残っていない**
（実測: 400本中 `custom-title` は4本、自動題名の行は0本）。
...
- **Codexは会話を確実に特定できない**。Claude Codeは `CLAUDE_CODE_SESSION_ID` を環境変数で渡してくるが、
  ...
  **ただし同じフォルダでClaudeも動いていると、当て推量は必ず外れる**——向こうは絶えず書いているので、
  時刻で選べば毎回そちらを掴む。呼ぶ側が `RELAY_HARNESS=codex relay` と名乗れば、その置き場だけを見る
  （`claude` / `codex`。知らない名前は名乗っていない扱い）。**どのハーネスの中に居るかは呼ぶ側しか
  知らない**ので、こちらからは推測しない
